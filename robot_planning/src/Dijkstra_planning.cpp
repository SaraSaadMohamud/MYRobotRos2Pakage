#include "robot_planning/Dijkstra_planning.hpp"
...
namespace robot_planning
{
    ...
    {
        RCLCPP_INFO(node_->get_logger(),"Clean up Plugin: %s of type Dijkstra planner",name_.c_str());
    }
    void  DijkstraPlanner::activate()
    {
        RCLCPP_INFO(node_->get_logger(),"Activate Plugin: %s of type Dijkstra planner",name_.c_str());
    }
    void DijkstraPlanner::deactivate()
    {
        RCLCPP_INFO(node_->get_logger(),"Deactivate Plugin: %s of type Dijkstra planner",name_.c_str());
    }
...
// Load Plugin During runtime
PLUGINLIB_EXPORT_CLASS(robot_planning::DijkstraPlanner, nav2_core::GlobalPlanner)